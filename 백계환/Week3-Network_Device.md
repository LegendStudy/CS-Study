...
- 상위 계층을 처리하는 기기는 하위 계층을 처리할 수 있지만 그 반대는 불가하다.
    - ex) L7 스위치는 그 밑의 모든 계층의 프로토콜을 처리할 수 있지만 AP(Access Point)는 물리 계층만 처리 가능
...
- 만약 장애가 발생한 서버가 있다면 이를 트래픽 분산 대상에서 제외해야 하는데, 이는 정기적으로 헬스 체크를 이용하여 감시하며 이루어진다.

> L4 스위치와 L7 스위치 차이
> 

L4 스위치도 로드밸런서 역할을 한다. L4 스위치는 전송 계층을 처리하는 기기로 스트리밍 관련 서비스에서는 사용할 수 없으며 메시지를 기반으로 인식하지 못하고 IP와 포트를 기반으로 트래픽을 분산한다. 
...
L2 스위치의 기능과 라우팅 기능을 갖춘 장비를 말한다. L3 스위치를 라우터라고 해도 무방하다.

라우터는 소프트웨어 기반 / 하드웨어 기반(L3 스위치)으로 나뉜다.
...
IP 주소 기반 라우팅은 불가능하며 단순히 패킷의 MAC 주소를 읽어 스위칭하는 역할을 한다. 목적지가 MAC 주소 테이블에 없다면 전체 포트에 전달하고 MAC 주소 테이블의 주소는 일정 시간 이후 삭제하는 기능도 있다.

## 브리지

bridge는 두 개의 근거리 통신망(LAN)을 상호 접속할 수 있도록 하는 통신망 연결 장치로, 포트와 포트 사이의 다리 역할을 하며 장치에서 받아온 MAC 주소를 MAC 주소 테이블로 관리한다.
...
이를 통해 패킷이 더 멀리 갈 수 있다. 하지만 광케이블이 보급됨에 따라 현재는 잘 쓰이지 않는다.
